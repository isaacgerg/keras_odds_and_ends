...
for i in range(nlayers):
    ...
    y = tf.layers.max_pooling2d(y_prepool, [pool_sizes[i], pool_sizes[i]], [pool_sizes[i], pool_sizes[i]])
    wheres[i] = getwhere([y_prepool, y])
...
train = tf.train.AdamOptimizer(learning_rate=0.001).minimize(loss_l2)
